// DO NOT EDIT. This is code generated via package:intl/generate_localized.dart
// This is a library that provides messages for a en locale. All the
// messages from the main program should be duplicated here with the same
// function name.

// Ignore issues from commonly used lints in this file.
// ignore_for_file:unnecessary_brace_in_string_interps, unnecessary_new
// ignore_for_file:prefer_single_quotes,comment_references, directives_ordering
// ignore_for_file:annotate_overrides,prefer_generic_function_type_aliases
// ignore_for_file:unused_import, file_names, avoid_escaping_inner_quotes

import 'package:intl/intl.dart';
import 'package:intl/message_lookup_by_library.dart';

final messages = new MessageLookup();

typedef String MessageIfAbsent(String messageStr, List<dynamic> args);

class MessageLookup extends MessageLookupByLibrary {
  String get localeName => 'en';

  static String m0(e) => " ${e} days ago";

  static String m1(error) => "Error checking favorite status: ${error}";

  static String m2(e) => "Error loading user data: ${e}";

  static String m3(error) => "Error sending password recovery email: ${error}";

  static String m4(e) => "Error signing out: ${e}";

  static String m5(error) => "Error updating favorite status: ${error}";

  static String m6(error) => "Error updating password: ${error}";

  static String m7(e) => "${e} is required";

  static String m8(e) => " ${e} hours ago";

  static String m9(e) => "Max Distance: ${e} Km";

  static String m10(e) => " ${e} months ago";

  static String m11(name) => "${name}\'s house";

  static String m12(e) => "Please enter ${e}";

  static String m13(price) => "${price} TND";

  static String m14(e) => " ${e} years ago";

  final messages = _notInlinedMessages(_notInlinedMessages);
  static Map<String, Function> _notInlinedMessages(_) => <String, Function>{
        "add": MessageLookupByLibrary.simpleMessage("Add"),
        "addCommentOptional":
            MessageLookupByLibrary.simpleMessage("Add a comment (optional)"),
        "addFeature": MessageLookupByLibrary.simpleMessage("Add Feature"),
        "addFeatures": MessageLookupByLibrary.simpleMessage("Add Features"),
        "addFurniture": MessageLookupByLibrary.simpleMessage("Add Furniture"),
        "addMainImage": MessageLookupByLibrary.simpleMessage("Add Main Image"),
        "addedToFavorites":
            MessageLookupByLibrary.simpleMessage("Added to favorites"),
        "additionalImages":
            MessageLookupByLibrary.simpleMessage("Additional Images"),
        "additionalInformation":
            MessageLookupByLibrary.simpleMessage("Additional Information"),
        "address": MessageLookupByLibrary.simpleMessage("Address"),
        "adjustSearchOrFilters": MessageLookupByLibrary.simpleMessage(
            "Try adjusting your search or filters"),
        "apply": MessageLookupByLibrary.simpleMessage("Apply"),
        "applyChanges": MessageLookupByLibrary.simpleMessage("Apply Changes"),
        "area": MessageLookupByLibrary.simpleMessage("Area"),
        "authFailed":
            MessageLookupByLibrary.simpleMessage("Authentication failed"),
        "available": MessageLookupByLibrary.simpleMessage("Available"),
        "availableHint": MessageLookupByLibrary.simpleMessage(
            "Enable this if your house is available"),
        "back": MessageLookupByLibrary.simpleMessage("Back"),
        "basicInformation":
            MessageLookupByLibrary.simpleMessage("Basic Information"),
        "bedrooms": MessageLookupByLibrary.simpleMessage("Bedrooms"),
        "camera": MessageLookupByLibrary.simpleMessage("Camera"),
        "cancel": MessageLookupByLibrary.simpleMessage("Cancel"),
        "changeRating": MessageLookupByLibrary.simpleMessage("Change Rate"),
        "changeToAnotherLoc":
            MessageLookupByLibrary.simpleMessage("change to another location"),
        "chooseAnImageSource":
            MessageLookupByLibrary.simpleMessage("Choose an image source"),
        "city": MessageLookupByLibrary.simpleMessage("City"),
        "comment": MessageLookupByLibrary.simpleMessage("Comment"),
        "confirmNewPassword":
            MessageLookupByLibrary.simpleMessage("Confirm New Password"),
        "contactSupport":
            MessageLookupByLibrary.simpleMessage("Contact Support"),
        "contactUsDirectly":
            MessageLookupByLibrary.simpleMessage("Or contact Us directly"),
        "daily": MessageLookupByLibrary.simpleMessage("Daily"),
        "dailyPayment": MessageLookupByLibrary.simpleMessage("Daily Payment"),
        "dans": MessageLookupByLibrary.simpleMessage("in"),
        "dark": MessageLookupByLibrary.simpleMessage("Dark"),
        "day": m0,
        "daysAgo": MessageLookupByLibrary.simpleMessage("d ago"),
        "delete": MessageLookupByLibrary.simpleMessage("Delete"),
        "deleteHouse": MessageLookupByLibrary.simpleMessage("Delete House"),
        "deleteHouseConfirmation": MessageLookupByLibrary.simpleMessage(
            "Are you sure you want to delete this house listing?"),
        "deleteRating": MessageLookupByLibrary.simpleMessage("Delete Rating"),
        "deleteRatingConfirmation": MessageLookupByLibrary.simpleMessage(
            "Are you sure you want to delete your rating?"),
        "describeIssue":
            MessageLookupByLibrary.simpleMessage("Describe your issue"),
        "dismiss": MessageLookupByLibrary.simpleMessage("Dismiss"),
        "distance": MessageLookupByLibrary.simpleMessage("Dist"),
        "editProfile":
            MessageLookupByLibrary.simpleMessage("Edit Personal Account"),
        "email": MessageLookupByLibrary.simpleMessage("Email"),
        "enableNotifications":
            MessageLookupByLibrary.simpleMessage("Enable Notifications"),
        "enterEmail": MessageLookupByLibrary.simpleMessage("Enter your email"),
        "enterName": MessageLookupByLibrary.simpleMessage("Enter your name"),
        "enterRentPrice":
            MessageLookupByLibrary.simpleMessage("Please enter a rent price"),
        "enterSalePrice":
            MessageLookupByLibrary.simpleMessage("Please enter a sale price"),
        "enterSubject":
            MessageLookupByLibrary.simpleMessage("Enter the subject"),
        "enterYourEmailToRecoverPassword": MessageLookupByLibrary.simpleMessage(
            "Enter your email to recover your password"),
        "errorCheckingFavoriteStatus": m1,
        "errorDeletingHouse":
            MessageLookupByLibrary.simpleMessage("Error deleting house"),
        "errorDeletingRating":
            MessageLookupByLibrary.simpleMessage("Error deleting rating"),
        "errorFetchingHouses": MessageLookupByLibrary.simpleMessage(
            "Error Fetching house details"),
        "errorLoadingUserData": m2,
        "errorSearchingForPlaces":
            MessageLookupByLibrary.simpleMessage("Error searching for places"),
        "errorSendingPasswordRecoveryEmail": m3,
        "errorSigningOut": m4,
        "errorSubmittingRating": MessageLookupByLibrary.simpleMessage(
            "Error submitting rating, try again later"),
        "errorUpdatingFavoriteStatus": m5,
        "errorUpdatingPassword": m6,
        "errorUpdatingProfile":
            MessageLookupByLibrary.simpleMessage("Error updating profile"),
        "errorUploadingHouse": MessageLookupByLibrary.simpleMessage(
            "Error uploading house listing"),
        "failedToLoadSearchResults": MessageLookupByLibrary.simpleMessage(
            "Failed to load search results"),
        "favouriteHouses":
            MessageLookupByLibrary.simpleMessage("Favorite Houses"),
        "fieldRequired": m7,
        "filters": MessageLookupByLibrary.simpleMessage("Filters"),
        "floor": MessageLookupByLibrary.simpleMessage("Floor"),
        "forRent": MessageLookupByLibrary.simpleMessage("For Rent"),
        "forSale": MessageLookupByLibrary.simpleMessage("For Sale"),
        "forgotPassword":
            MessageLookupByLibrary.simpleMessage("Forgot Password?"),
        "furnished": MessageLookupByLibrary.simpleMessage("Furnished"),
        "furniture": MessageLookupByLibrary.simpleMessage("Furniture"),
        "furnitureItems":
            MessageLookupByLibrary.simpleMessage("Fridge, TV, Table...."),
        "gallery": MessageLookupByLibrary.simpleMessage("Gallery"),
        "garageFeatures": MessageLookupByLibrary.simpleMessage(
            "Garage, 2 Rooms, 2 Toilets..."),
        "genderFemale": MessageLookupByLibrary.simpleMessage("Female"),
        "genderMale": MessageLookupByLibrary.simpleMessage("Male"),
        "genderOther": MessageLookupByLibrary.simpleMessage("Other"),
        "groundFloor":
            MessageLookupByLibrary.simpleMessage("0 for ground floor"),
        "has3DView": MessageLookupByLibrary.simpleMessage("3D View Available"),
        "hasLivingRoom": MessageLookupByLibrary.simpleMessage(
            "The house has a living room ?"),
        "hasParking":
            MessageLookupByLibrary.simpleMessage("The house has a parking ?"),
        "hasWifi":
            MessageLookupByLibrary.simpleMessage("Does it have a WiFi ?"),
        "hintText":
            MessageLookupByLibrary.simpleMessage("villa, apartment, studio..."),
        "home": MessageLookupByLibrary.simpleMessage("Home"),
        "hour": m8,
        "hoursAgo": MessageLookupByLibrary.simpleMessage("h ago"),
        "houseDeleteSuccess":
            MessageLookupByLibrary.simpleMessage("House deleted successfully!"),
        "houseSpecifications":
            MessageLookupByLibrary.simpleMessage("House Specifications"),
        "houseUploadedSuccess": MessageLookupByLibrary.simpleMessage(
            "House listing uploaded successfully!"),
        "inbox": MessageLookupByLibrary.simpleMessage("Inbox"),
        "invalidEmail":
            MessageLookupByLibrary.simpleMessage("Enter a valid email address"),
        "isAvailable": MessageLookupByLibrary.simpleMessage("Is Available"),
        "isFurnished":
            MessageLookupByLibrary.simpleMessage("Is it Furnished ?"),
        "justContinue": MessageLookupByLibrary.simpleMessage("Continue"),
        "justCurrent": MessageLookupByLibrary.simpleMessage("Current"),
        "justIn": MessageLookupByLibrary.simpleMessage("in"),
        "language": MessageLookupByLibrary.simpleMessage("Language"),
        "lastName": MessageLookupByLibrary.simpleMessage("Last Name"),
        "light": MessageLookupByLibrary.simpleMessage("Light"),
        "livingRoom": MessageLookupByLibrary.simpleMessage("Living Room"),
        "loading": MessageLookupByLibrary.simpleMessage("Loading..."),
        "loadingMessages":
            MessageLookupByLibrary.simpleMessage("Loading messages..."),
        "loadingNotifications":
            MessageLookupByLibrary.simpleMessage("Loading notifications..."),
        "location": MessageLookupByLibrary.simpleMessage("Location"),
        "logout": MessageLookupByLibrary.simpleMessage("Logout"),
        "logoutConfirmation": MessageLookupByLibrary.simpleMessage(
            "Are you sure you want to logout?"),
        "main": MessageLookupByLibrary.simpleMessage("Main"),
        "mainImage": MessageLookupByLibrary.simpleMessage("Main Image"),
        "maxDistance": m9,
        "maxPrice": MessageLookupByLibrary.simpleMessage("Max Price"),
        "message": MessageLookupByLibrary.simpleMessage("Message"),
        "messages": MessageLookupByLibrary.simpleMessage("Messages"),
        "middleName":
            MessageLookupByLibrary.simpleMessage("Middle Name (optional)"),
        "minPrice": MessageLookupByLibrary.simpleMessage("Min Price"),
        "minutesAgo": MessageLookupByLibrary.simpleMessage("m ago"),
        "month": m10,
        "monthly": MessageLookupByLibrary.simpleMessage("Monthly"),
        "monthlyPayment":
            MessageLookupByLibrary.simpleMessage("Monthly Payment"),
        "monthsAgo": MessageLookupByLibrary.simpleMessage("mo ago"),
        "moreDetails": MessageLookupByLibrary.simpleMessage("More Details"),
        "myHouses": MessageLookupByLibrary.simpleMessage("My Houses"),
        "name": MessageLookupByLibrary.simpleMessage("Name"),
        "newPassword": MessageLookupByLibrary.simpleMessage("New Password"),
        "no": MessageLookupByLibrary.simpleMessage("No"),
        "noAdditionalImages": MessageLookupByLibrary.simpleMessage(
            "No additional images selected"),
        "noHousesListed": MessageLookupByLibrary.simpleMessage(
            "You don\'t have any houses listed."),
        "noMessages": MessageLookupByLibrary.simpleMessage("No messages"),
        "noMoreHousesFound":
            MessageLookupByLibrary.simpleMessage("No more houses found"),
        "noNotifications":
            MessageLookupByLibrary.simpleMessage("No notifications"),
        "noRatings": MessageLookupByLibrary.simpleMessage("No ratings yet"),
        "noResultsFound":
            MessageLookupByLibrary.simpleMessage("No results found"),
        "noRooms": MessageLookupByLibrary.simpleMessage("0 if no rooms"),
        "notAvailable": MessageLookupByLibrary.simpleMessage("Not Available"),
        "notLoggedIn": MessageLookupByLibrary.simpleMessage("Not logged in..."),
        "notifications": MessageLookupByLibrary.simpleMessage("Notifications"),
        "oldPassword": MessageLookupByLibrary.simpleMessage("Old Password"),
        "options": MessageLookupByLibrary.simpleMessage("Options"),
        "ownerComment": MessageLookupByLibrary.simpleMessage("Owner Comment"),
        "ownerHouse": m11,
        "ownerInfoMessage": MessageLookupByLibrary.simpleMessage(
            "Owner information will be automatically filled based on your account details."),
        "ownerInformation":
            MessageLookupByLibrary.simpleMessage("Owner Information"),
        "password": MessageLookupByLibrary.simpleMessage("Password"),
        "passwordRecoveryEmailSentSuccessfully":
            MessageLookupByLibrary.simpleMessage(
                "Password recovery email sent successfully"),
        "passwordUpdatedSuccessfully": MessageLookupByLibrary.simpleMessage(
            "Password updated successfully"),
        "passwordsDoNotMatch":
            MessageLookupByLibrary.simpleMessage("Passwords do not match"),
        "payment": MessageLookupByLibrary.simpleMessage("Payment"),
        "personalAccount":
            MessageLookupByLibrary.simpleMessage("Personal Account"),
        "phone": MessageLookupByLibrary.simpleMessage("Phone"),
        "phoneNumber": MessageLookupByLibrary.simpleMessage("Phone Number"),
        "phoneUpdateSuccess": MessageLookupByLibrary.simpleMessage(
            "Phone number updated successfully"),
        "pickCustomLocation":
            MessageLookupByLibrary.simpleMessage("Pick Custom Location"),
        "pleaseEnter": m12,
        "pleaseEnterPhoneNumber":
            MessageLookupByLibrary.simpleMessage("Please enter a phone number"),
        "pleaseEnterValidPhoneNumber": MessageLookupByLibrary.simpleMessage(
            "Please enter a valid phone number"),
        "price": MessageLookupByLibrary.simpleMessage("Price"),
        "priceRange": MessageLookupByLibrary.simpleMessage("Price Range"),
        "priceValue": m13,
        "profile": MessageLookupByLibrary.simpleMessage("Profile"),
        "profileUpdatedSuccessfully": MessageLookupByLibrary.simpleMessage(
            "Profile updated successfully"),
        "rate": MessageLookupByLibrary.simpleMessage("Rate"),
        "rateThisHouse":
            MessageLookupByLibrary.simpleMessage("Rate this house"),
        "rating": MessageLookupByLibrary.simpleMessage("Rating"),
        "ratingDeleted":
            MessageLookupByLibrary.simpleMessage("Rating deleted successfully"),
        "ratingMultiple": MessageLookupByLibrary.simpleMessage("ratings"),
        "ratingSingle": MessageLookupByLibrary.simpleMessage("rating"),
        "ratingSubmitted": MessageLookupByLibrary.simpleMessage(
            "Rating submitted successfully"),
        "recenter": MessageLookupByLibrary.simpleMessage("recenter"),
        "recently": MessageLookupByLibrary.simpleMessage("Recently"),
        "removedFromFavorites":
            MessageLookupByLibrary.simpleMessage("Removed from favorites"),
        "rentPrice": MessageLookupByLibrary.simpleMessage("Rent Price"),
        "reset": MessageLookupByLibrary.simpleMessage("Reset"),
        "reviews": MessageLookupByLibrary.simpleMessage("Reviews"),
        "salePrice": MessageLookupByLibrary.simpleMessage("Sale Price"),
        "saveChanges": MessageLookupByLibrary.simpleMessage("Save Changes"),
        "search": MessageLookupByLibrary.simpleMessage("Search"),
        "selectBirthdate":
            MessageLookupByLibrary.simpleMessage("Select Birthdate"),
        "selectDistrict":
            MessageLookupByLibrary.simpleMessage("Select District"),
        "selectGender": MessageLookupByLibrary.simpleMessage("Select Gender"),
        "selectImageSource":
            MessageLookupByLibrary.simpleMessage("Select Image Source"),
        "selectRating":
            MessageLookupByLibrary.simpleMessage("Please select a rating"),
        "selectRegion": MessageLookupByLibrary.simpleMessage("Select Region"),
        "selectYourPhoneCode":
            MessageLookupByLibrary.simpleMessage("Select your phone code"),
        "sendRecoveryEmail":
            MessageLookupByLibrary.simpleMessage("Send Recovery Email"),
        "settings": MessageLookupByLibrary.simpleMessage("Settings"),
        "sortBy": MessageLookupByLibrary.simpleMessage("Sort by:"),
        "sorting": MessageLookupByLibrary.simpleMessage("Sorting"),
        "status": MessageLookupByLibrary.simpleMessage("Status"),
        "subject": MessageLookupByLibrary.simpleMessage("Subject"),
        "submit": MessageLookupByLibrary.simpleMessage("Submit"),
        "supportRequestFailed": MessageLookupByLibrary.simpleMessage(
            "Failed to send support request. Please try again."),
        "supportRequestSent": MessageLookupByLibrary.simpleMessage(
            "Support request sent successfully!"),
        "themeMode": MessageLookupByLibrary.simpleMessage("Theme Mode"),
        "type": MessageLookupByLibrary.simpleMessage("Type"),
        "typeYourPasswordToApplyChanges": MessageLookupByLibrary.simpleMessage(
            "Type your password to apply changes"),
        "uhOhPageNotFound":
            MessageLookupByLibrary.simpleMessage("uh-oh!\nPage not found"),
        "update": MessageLookupByLibrary.simpleMessage("Update"),
        "updateError": MessageLookupByLibrary.simpleMessage("Error"),
        "updateHouseDetails":
            MessageLookupByLibrary.simpleMessage("Update House Details"),
        "uploadHouseListing":
            MessageLookupByLibrary.simpleMessage("Upload House Listing"),
        "useCurrentLocation":
            MessageLookupByLibrary.simpleMessage("Use Current Location"),
        "validPhoneNumberRequired": MessageLookupByLibrary.simpleMessage(
            "Please enter a valid phone number"),
        "verificationCodeError": MessageLookupByLibrary.simpleMessage(
            "Error sending verification code"),
        "weReUploadingWait": MessageLookupByLibrary.simpleMessage(
            "We\'re uploading your House, please wait..."),
        "wrongPassword": MessageLookupByLibrary.simpleMessage("Wrong password"),
        "year": m14,
        "yearsAgo": MessageLookupByLibrary.simpleMessage("y ago"),
        "yes": MessageLookupByLibrary.simpleMessage("Yes")
      };
}
